# Задание 2 (Разработка): VPN-тема + тесты на Jest

Проект — демонстрационный мини‑модуль “VPN connection manager” (логика выбора сервера, валидация конфигурации, kill‑switch, состояния подключения) + тесты на **Jest**:

- **10+ unit tests** (папка `tests/unit`)
- **2 functional tests** (папка `tests/functional`)

Важно: это учебный проект под тему VPN (не настоящий VPN‑клиент, а доменная логика + симуляция подключения).

## Стек

- Node.js 18+ (у тебя Node уже есть)
- Jest
...
## Что считается unit tests (примеры)
...
## Что считается functional tests (примеры)
...
2) Сделай скриншот консоли, где видно итог (`PASS ...`, количество тестов, и что тесты прошли).
3) Сохрани скриншот сюда:
`docs/screenshots/test_results.png` (или `.jpg`)

4) В README оставь картинку (если файл назвал иначе — поправь путь):
...
- `docs/screenshots/` — место для скриншотов результатов
